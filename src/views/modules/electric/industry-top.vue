<template>
  <div class="industry-top common">
    <div class="title">{{regionName}}各行业异常用电排名前十用户信息表</div>
...
export default {
  ...
  activated(){
    // 获取传递过来的参数-- start
    if(sessionStorage.getItem('selectDateMapToIndustryTop')){
      this.selectDate = JSON.parse(sessionStorage.getItem('selectDateMapToIndustryTop'))
    }
    if(sessionStorage.getItem('assessmentMapToIndustryTop')){
      this.assessment = JSON.parse(sessionStorage.getItem('assessmentMapToIndustryTop'))
    }
    if(sessionStorage.getItem('detectionDayMapToIndustryTop')){
      this.detectionDay = sessionStorage.getItem('detectionDayMapToIndustryTop')
    }
    // 获取参数过来的参数-- end
  ...
  destroyed(){
    ...
    sessionStorage.setItem('assessmentMapToIndustryTop','')
  },
  methods: {
    getDataList() {
      this.dataListLoading = true
      this.$http({
        url: this.$http.adornUrl('/electric/homeIndustry/industry'),
        method: 'get',
    ...
    ignoreDetailsHandle(electricUserNumber,electricUserInstrument){
      ...
      this.$nextTick(()=>{
        this.$refs['ignoreDetails'].init(electricUserNumber,electricUserInstrument,'')
      })
    },
    // 派单详情
