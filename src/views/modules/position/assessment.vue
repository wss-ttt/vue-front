<template>
  <div class="mod-assessment common">
      ...
      <ul>
        ...
        <li>
          <button type="button" class="bg-btn" @click="exportData"
            v-if="isAuth('position:positionassessresult:export')">导出</button>
        </li>
      </ul>
    </div>
      ...
      <el-table-column fixed="right" header-align="center" align="center" width="150" label="操作">
        <template slot-scope="scope">
          <el-button v-if="scope.row.tailAfterStatus !== 1" type="text"
            size="small" @click.stop="track(scope.row.id, scope.row.positionCode)">
            异常跟踪</el-button>
    ...
    <!-- 异常评估折线图 -->
    <el-dialog :title="errorLine.positionCode + ' 异常评估折线图'" :visible.sync="errorLine.visible" width="40%">
  ...
  export default {
    ...
    methods: {
      ...
      getDataList() {
        ...
        this.$http({
          url: this.$http.adornUrl('/position/positionassessresult/listPage'),
          method: 'get',
      ...
      exportData() {
        exportDataToExcel('/position/positionassessresult/export', this.dataForm).then(() => {}).catch(err => {
      ...
      // 打开误差折线图
      openErrorLine(row) {
        this.errorLine.positionCode = row.positionCode // 站点编号
        this.errorLine.positionName = row.positionName // 站点名称
      ...
      getErrorLineData() {
        return new Promise((resolve, reject) => {
          this.$http({
            url: this.$http.adornUrl('/position/positionassessresult/abnormalProbability'),
            method: 'get',
            params: this.$http.adornParams({
              positionCode: this.errorLine.positionCode, // 站点编号
      ...
      track(id, positionCode) {
        this.$confirm('确认进行异常跟踪操作吗？', '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/position/positionassessresult/updatePositionRecordTail'),
            method: 'post',
