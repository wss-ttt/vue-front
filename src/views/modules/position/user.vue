  ...
  <div class="mod-user common">
    <div class="query-area">
      <ul>
        ...
        <li>
          <span>站点名称:</span>
          <el-input v-model="dataForm.stationName" placeholder="站点名称" clearable style="width:200px;">
          </el-input>
        </li>
        <li>
          <span>计量点编号:</span>
        ...
        <li>
          <el-upload action="#" ref="fileupload"
            ...
            <el-tooltip effect="dark" content="只能上传xls、xslx文件" placement="bottom">
              <button v-if="isAuth('record:positionrecord:import')" class="bg-btn">导入</button>
            </el-tooltip>
            <span v-if="isAuth('record:positionrecord:import')" style="color:#ccc;">注意:只能上传xls、xslx文件</span>
          </el-upload>
        </li>
      </ul>
    </div>
  ...
  export default {
    data() {
      return {
        ...
        url: '/record/positionrecord/listPage'
      }
    },
    components: {},
    mounted() {
    ...
    methods: {
      ...
      uploadFile(item) {
        ...
        this.$http({
            url: this.$http.adornUrl('/record/positionrecord/positionImport'),
            method: 'post',
            data: form
          })
          .then(({
      ...
      downloadTemplate() {
        this.$http({
          ...
          excelDownload(data, '计量点档案模板.xlsx')
        }).catch(() => {
