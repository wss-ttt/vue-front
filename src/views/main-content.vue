...
export default {
  ...
  methods: {
    ...
    removeTabHandle(tabName) {
      ...
      if (tabName === this.homePageName) return
      ...
      // 删除tab标签页
      this.mainTabs = tabs.filter(tab => tab.name !== tabName)
    ...
    tabsCloseCurrentHandle() {
      if (this.mainTabsActiveName === this.homePageName) return
    ...
    tabsCloseOtherHandle() {
      this.mainTabs = this.mainTabs.filter(
        item =>
          item.name === this.mainTabsActiveName ||
          item.name === this.homePageName
      )
    },
    // tabs, 关闭全部
    tabsCloseAllHandle() {
      ...
      // 清空操作
      this.mainTabs = []
      // 如果当前本身就处在this.homePageName上
      let route = this.$route
      if (route.name === this.homePageName) {
        let tab = {
